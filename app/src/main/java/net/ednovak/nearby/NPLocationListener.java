...
public class NPLocationListener implements LocationListener {
	private final static String TAG = NPLocationListener.class.getName();
	...
	private static NPLocationListener instance = null;

	private NPLocationListener(Context newCtx){
	...
	public static NPLocationListener getInstance(Context newCtx){
		if(instance == null){
			instance = new NPLocationListener(newCtx);
		}
		return instance;
	}
	...
	public void onLocationChanged(Location newLocation) { // The callback
		if (newLocation.hasAccuracy()){
			...
			else if(NPLib.getTimeSince(bestLoc.getTime()) > 5000) { // If it's been more than five seconds
