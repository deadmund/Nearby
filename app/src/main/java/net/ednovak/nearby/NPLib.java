 ...
 */
public class NPLib {
    private final static String TAG = NPLib.class.getName();
    ...
    public static String prettyArray(int[][] input, char delim, char bigDelim){
        // responseData[i][0] = NPLib.boolToInt(aliceBF[0].getBit(indexQuery[i]));
        // responseData[i][1] = NPLib.boolToInt(r.nextBoolean());
        StringBuilder sb = new StringBuilder();
        for(int i = 0; i < input.length; i++){
            sb.append(input[i][0]);
            sb.append(delim);
