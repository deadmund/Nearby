 ...
 */
public class NPSocket extends Socket {
    private final static String TAG = NPSocket.class.getName();
    ...
    NPSocket(InetAddress addr, int port) throws IOException{
        super(addr, port);
    ...
    public byte[] readSocket(){
        ...
        while(sumBytes < BYTE_COUNT){
            ...
            try{
                ...
                //Log.d(TAG, "Last Buffer Boundry Section : " + NPLib.section(buffer, sumBytes - (newAmount + 3), sumBytes - (newAmount - 3)));
                //Log.d(TAG, "current buffer boundry Section: " + NPLib.section(buffer, sumBytes - 3, sumBytes + 3));
